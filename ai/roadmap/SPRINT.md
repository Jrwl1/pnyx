...
| S1-T07 | Harden registration so public self-service cannot assign privileged roles. | `/auth/register` only produces `user` role for public caller path; privileged role attempts are rejected or normalized with explicit response contract. | `pnpm test -- -t "register role hardening"` | Fixer |
