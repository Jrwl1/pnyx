...
- Browser and manual verification are strong, but launch confidence still needs durable automated coverage for critical public, contributor, moderator, and editorial flows; the sprint is now re-scoped to allow dependency-backed browser automation because the repo-native Windows harness path was not reliable enough.
...
3. S-29 is unblocked at the planning level, but the dependency-backed browser automation path still has to be implemented and proven.
