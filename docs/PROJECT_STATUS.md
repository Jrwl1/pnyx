...
1. Real party, membership, and canonical promise entities do not exist yet, so party pages and trust surfaces still rely on placeholder or statement-only data.
...
1. Execute `S-22` to replace frontend-only party placeholders with backend-backed party, alias, and membership data.
2. Execute `S-23` to introduce canonical promises while keeping current statement-backed public reads compatible.
3. Execute `S-24` through `S-26` to add claim canonization, trust dimensions, release hardening, and a full UI audit/manual verification pass.
